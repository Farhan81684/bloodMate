pipeline {
    ...
    environment{
        ...
        BACKEND_IMAGE = "bloodmate-backend"
        K8S_CRED = "k8s-config"
    ...
    stages {
        stage('1st Git CheckOut') {
            steps {
                git branch: 'main', credentialsId: '17', url: 'https://github.com/Farhan81684/bloodMate.git'
            }
        }
        ...
        stage('3 Deploy to Kubernates') {
            steps {
                script{
                    ...
                    sh 'kubectl rollout restart deployment bloodmate-backend'
                    }
                }
            }
        }
